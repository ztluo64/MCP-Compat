...
fintune_path = config.LOCATION_FINETUNED_MODEL
...
tar_files = sorted(glob.glob(os.path.join(config.LOCATION_WEBDATASET_DIR, 'train-*.tar')))
...
def extract_predicted_label(predicted_response):
    """Extract the Location label from the Summary section"""
    
    # Try to find "Summary:" section and extract Location conclusion
    ...
    # Fallback 1: Look for "Location Conclusion:" 
    conclusion_pattern = r'Location Conclusion:\s*(In-context|Out-of-context)'
...
# ============================================================================
# 建立顺序索引到COCO image_id的映射
# ============================================================================

def build_index_mapping():
    """建立dataset顺序索引 -> COCO image_id的映射"""
    
    print("\n" + "="*80)
    print("Building Index Mapping: Dataset Index -> COCO Image ID")
    print("="*80)
    
    from pycocotools.coco import COCO
    
    # 加载注释
    train_ann = config.COCO_TRAIN_ANNOTATIONS
    val_ann = config.COCO_VAL_ANNOTATIONS
    ...
    print(f"Train images: {len(train_img_ids)}")
    print(f"Val images: {len(val_img_ids)}")
    
    # 建立映射：dataset_index -> (image_id, source, coco_object)
    index_to_info = {}
    
    # Train集的映射
    for idx, img_id in enumerate(train_img_ids):
        index_to_info[idx] = {
            'image_id': img_id,
            'source': 'train',
            'coco': coco_train,
    ...
    # Val集的映射（接着train的索引继续）
    train_len = len(train_img_ids)
    for idx, img_id in enumerate(val_img_ids):
        index_to_info[train_len + idx] = {
            'image_id': img_id,
            'source': 'val',
            'coco': coco_val,
            'img_path': config.COCO_VAL_IMAGES
        }
    
    print(f"\n✅ Built mapping for {len(index_to_info)} images")
    print(f"Index range: 0 to {len(index_to_info)-1}")
    
    # 验证前几个映射
    print("\n🔍 Sample mappings:")
    for i in [0, 1, 100, train_len, train_len+1]:
        if i in index_to_info:
            info = index_to_info[i]
            print(f"  Dataset Index {i} -> Image ID {info['image_id']} ({info['source']})")
    
    return index_to_info, coco_train, coco_val
...
def test_original_images_with_metrics(model, processor, test_dataset, coco_data, coco, num_samples=20):
    ...
    for i in tqdm(range(len(test_dataset)), desc="Testing"):
        ...
        # 生成prompt
        user_prompt = generate_location_prompt(actual_object_name)
