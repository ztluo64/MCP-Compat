...
# coding: utf-8
"""
测试 Qwen2.5-VL-3B (LoRA Finetuned) 在所有 common 图片上的表现
...
if __name__ == "__main__":
    ...
    common_results_df.to_csv(os.path.join(output_dir, 'common_all_inference_results_lora.csv'), index=False)
    with open(os.path.join(output_dir, 'common_all_inference_metrics_lora.json'), 'w') as f:
    ...
    # 保存统计结果
    stats_df.to_csv(os.path.join(output_dir, 'common_pair_statistics_lora.csv'), index=False)
    ...
    print(f"   - common_all_inference_results_lora.csv (完整预测结果)")
    print(f"   - common_all_inference_metrics_lora.json (评估指标)")
    print(f"   - common_pair_statistics_lora.csv (category-location pair 统计)")
