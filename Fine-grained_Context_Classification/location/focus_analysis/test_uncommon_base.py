...
# coding: utf-8
"""
测试 Qwen2.5-VL-3B (Base Model) 在所有 uncommon 图片上的表现
...
if __name__ == "__main__":
    ...
    uncommon_results_df.to_csv(os.path.join(output_dir, 'uncommon_all_inference_results_base.csv'), index=False)
    with open(os.path.join(output_dir, 'uncommon_all_inference_metrics_base.json'), 'w') as f:
    ...
    # 保存统计结果
    stats_df.to_csv(os.path.join(output_dir, 'uncommon_pair_statistics_base.csv'), index=False)
    ...
    print(f"   - uncommon_all_inference_results_base.csv (完整预测结果)")
    print(f"   - uncommon_all_inference_metrics_base.json (评估指标)")
    print(f"   - uncommon_pair_statistics_base.csv (category-location pair 统计)")