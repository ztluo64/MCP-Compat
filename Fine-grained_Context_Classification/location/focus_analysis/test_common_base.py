...
# coding: utf-8
"""
测试 Qwen2.5-VL-3B (Base Model) 在所有 common 图片上的表现
- common_images: ground truth = In-context
...
def load_and_sample_data():
    """加载CSV，返回所有common数据（过滤掉snow）"""
    data_dir = CONFIG['data_dir']
    
    # 读取CSV
    common_df = pd.read_csv(os.path.join(data_dir, 'common_images.csv'))
    
    print(f"Original - Common: {len(common_df)}")
    
    # 过滤掉 location 为 snow 的样本
    common_df = common_df[common_df['location'] != 'snow']
    
    print(f"After removing snow - Common: {len(common_df)}")
    
    # 添加标签和图片路径
    common_df = common_df.copy()
    common_df['ground_truth'] = 'In-context'
    common_df['image_path'] = common_df['new_filename'].apply(
        lambda x: os.path.join(data_dir, 'common_images', x)
    )
    
    # shuffle
    common_df = common_df.sample(frac=1, random_state=CONFIG['seed']).reset_index(drop=True)
    
    print(f"✅ Total common samples: {len(common_df)}")
    
    # 打印 category-location pair 分布
    print(f"\n📊 Category-Location distribution:")
    pair_counts = common_df.groupby(['category', 'location']).size().reset_index(name='count')
...
if __name__ == "__main__":
    ...
    # 加载所有 common 数据
    common_df = load_and_sample_data()
    
    output_dir = CONFIG['data_dir']
    
    # ========== 运行所有 common (In-context) ==========
    print("\n" + "="*80)
    print("📍 Running ALL COMMON images (Ground Truth: In-context)")
    print("="*80)
    
    common_results, common_time_stats = run_inference(common_df, dataset_name="COMMON")
    common_metrics = evaluate_results(common_results)
    common_metrics['time_stats'] = common_time_stats
    
    # 保存完整结果
    common_results_df = pd.DataFrame(common_results)
    common_results_df.to_csv(os.path.join(output_dir, 'common_all_inference_results_base.csv'), index=False)
    with open(os.path.join(output_dir, 'common_all_inference_metrics_base.json'), 'w') as f:
    ...
    # 创建统计 DataFrame
    stats_df = common_results_df.groupby(['category', 'location']).agg(
    ...
    # 计算比例
    stats_df['in_context_rate'] = stats_df['pred_in_context'] / stats_df['total']
    stats_df['accuracy'] = stats_df['correct'] / stats_df['total']
    
    # 按 accuracy 降序排列
    stats_df = stats_df.sort_values('accuracy', ascending=False)
    
    print("\n🟢 Pairs performance (descending by accuracy):")
    print("-" * 90)
    print(f"{'Category':<12} {'Location':<12} {'Total':>8} {'Pred IC':>10} {'Pred OOC':>10} {'IC Rate':>10} {'Accuracy':>10}")
    print("-" * 90)
    for _, row in stats_df.iterrows():
        print(f"{row['category']:<12} {row['location']:<12} {row['total']:>8} {row['pred_in_context']:>10} {row['pred_out_of_context']:>10} {row['in_context_rate']:>10.2%} {row['accuracy']:>10.2%}")
    print("-" * 90)
    
    # 保存统计结果
    stats_df.to_csv(os.path.join(output_dir, 'common_pair_statistics_base.csv'), index=False)
    
    # ========== 总结 ==========
    overall_time = time.time() - overall_start
    
    print("\n" + "="*80)
    print("📊 FINAL SUMMARY (Base Model)")
    print("="*80)
    
    print(f"\n🟢 COMMON (In-context):")
    print(f"   Total samples: {len(common_results)}")
    print(f"   Accuracy: {common_metrics['accuracy']:.2%}")
    print(f"   Time: {common_time_stats['total_time']:.1f}s ({common_time_stats['total_time']/60:.1f} min)")
    
    print(f"\n📈 Top 5 pairs with HIGHEST accuracy:")
    for i, (_, row) in enumerate(stats_df.head(5).iterrows()):
        print(f"   {i+1}. {row['category']}-{row['location']}: {row['accuracy']:.2%} ({row['correct']}/{row['total']})")
    
    print(f"\n📉 Top 5 pairs with LOWEST accuracy:")
    for i, (_, row) in enumerate(stats_df.tail(5).iloc[::-1].iterrows()):
        print(f"   {i+1}. {row['category']}-{row['location']}: {row['accuracy']:.2%} ({row['correct']}/{row['total']})")
    
    print(f"\n⏱️  Total time: {overall_time:.1f}s ({overall_time/60:.1f} min)")
    print(f"   Avg per sample: {overall_time/len(common_results):.2f}s")
    
    print(f"\n✅ All results saved to {output_dir}")
    print(f"   - common_all_inference_results_base.csv (完整预测结果)")
    print(f"   - common_all_inference_metrics_base.json (评估指标)")
    print(f"   - common_pair_statistics_base.csv (category-location pair 统计)")
