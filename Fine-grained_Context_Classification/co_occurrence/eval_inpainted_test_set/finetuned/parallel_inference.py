...
def extract_predicted_label(predicted_response):
    """Extract the Co_occurrence label from the Summary section"""
    
    # Try to find "Summary:" section and extract Co_occurrence conclusion
...
def load_model_on_gpu(gpu_id, model_path, use_4bit=True, use_lora=True):
    ...
    model = PeftModel.from_pretrained(
         model,
        config.CO_OCCURRENCE_FINETUNED_MODEL,
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # Generate prompt
                user_prompt = generate_co_occurrence_prompt(object_name)
