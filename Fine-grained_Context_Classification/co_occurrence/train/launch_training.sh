...
# Multi-GPU Training Launch Script for Qwen2.5-VL-3B Co-occurrence Fine-tuning
# Works with or without accelerate config file

echo "🚀 Starting Qwen2.5-VL-3B Co-occurrence Multi-GPU Training on 4x H100"
echo "============================================"
echo "Timestamp: $(date)"
...
mkdir -p ./qwen25_finetuned_co_occurrence_specialist_multi_gpu

TRAINING_SCRIPT="train_multi_gpu.py"
...
if [ "$LAUNCHED" = true ]; then
    if [ $EXIT_CODE -eq 0 ]; then
        ...
        echo "Timestamp: $(date)"
        echo ""
        echo "Check the log files in ./qwen25_finetuned_co_occurrence_specialist_multi_gpu/logs/"
        exit $EXIT_CODE
    fi
else
    echo -e "\n============================================"
